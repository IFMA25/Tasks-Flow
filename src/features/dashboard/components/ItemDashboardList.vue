...
<template>
  <li class="grid pt-2 pb-4 border-t border-default gap-y-2 items-center
  ...
  <div class="[grid-area:meta] flex items-center gap-4 text-xs leading-[1.3]">
    ...
    <p class="flex items-center gap-1 before:content-[''] before:block before:w-1 before:h-1" :class="colorsPriority[task.priority]">
      <span class="capitalize">{{ task.priority }}</span> priority
    </p>
  </div>

  <ul v-if="task.tags.length" class="[grid-area:tags] flex gap-2">
    <li
